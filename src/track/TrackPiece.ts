...
export class TrackPiece {
  ...
  private buildTurn() {
    ...
    this.addWalls();
    this.addTurnMarkings();
  }

  private addTurnMarkings() {
    const hw = ROAD_WIDTH / 2;
    const mat = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.15 });
    // Two diagonal dashes at the bend
    for (let i = 0; i < 3; i++) {
      const m = new THREE.Mesh(new THREE.PlaneGeometry(0.3, 2.5), mat);
      m.position.set(0.5 + i * 0.6, ROAD_THICKNESS + 0.001, 0.5 + i * 0.6);
